...
class Day1:
    def __init__(self):
        ...
        # counter of measurements larger than previous one for the first part
        self.measurement_counter = 0
        # pretty self-explanatory, this is the counter that will provide the solution for the second part
        self.larger_sum_counter = 0

        self.part1()
        self.part2()

        print(f'Day 1, Part 1: {self.measurement_counter}')
        print(f'Day 1, Part 2: {self.larger_sum_counter}')
    ...
    def part1(self):
        ...
        for i, element in enumerate(self.data):
            ...
            aux = int(element)
            self.input_int.append(aux)

            # in order to compare the elements, the first one must be available upfront
            # thus this loop will be available starting with the second element
            if i != 0 and self.input_int[i] > self.input_int[i - 1]:
    ...
    def part2(self):
        ...
        # let's initialize the first sum with the first three elements
        first_sum = self.input_int[0] + self.input_int[1] + self.input_int[2]

        # this will be the current sum to be compared to previous one
        next_sum = 0

        # main calculation loop
        for i, element in enumerate(self.input_int):
            # already calculated the first sum of the first three elements
            # thus, we have to start making new sums starting in the fourth element
            if i > 2:
                next_sum = element + self.input_int[i - 1] + self.input_int[i - 2]
                if next_sum > first_sum:
                    self.larger_sum_counter += 1
                first_sum = next_sum
